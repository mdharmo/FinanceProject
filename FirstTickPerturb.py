...
def main(weekcount, ticknum, winnum, large, restart):
    ...
    mainadd = '/home/mharmon/FinanceProject/ModelResults/tickconvpert' + str(ticknum) + 'win' + str(winnum)
    ...
    for overall in range(large, 4):
        ...
        for i in range(weekcount, numtests):
            ...
            for w in range(len(temp_weights)):
                new_weight = np.copy(temp_weights[w])
                new_weight =  np.random.uniform(low=-np.std(temp_weights[w])/4,high=np.std(temp_weights[w])/4,size =new_weight.shape)+new_weight
                temp_weights[w]=new_weight
