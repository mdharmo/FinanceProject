...
def main(weekcount,ticknum,winnum,large,restart):
    ...
    old_numtests=1
    mainadd = '/home/mharmon/FinanceProject/ModelResults/ticksamptest'+str(ticknum) + 'win' + str(winnum)
    datamain = '/home/mharmon/FinanceProject/Data/tickdata/trainnewday'
    batchsize=256
    seqsize = ticknum
    model = build_rnn(seqsize,batchsize,labseq,stocks)
    month_len = int(8064)
    week = int((month_len)/4.)
    # Need to additionally fit the smotes for training purposes...
    for overall in range(large,large+1):
        address  = datamain+str(ticknum) + 'win'+str(winnum)+'sector'+str(overall)+'.pkl'
        ...
        if overall>0 and restart==1 and weekcount>=4:
            ...
            print(address)
            old_add = datamain+str(ticknum) + 'win'+str(winnum)+'sector'+str(overall-1)+'.pkl'
        ...
        if weekcount<=4 and restart==1 and overall>0:
            print('Old data is from')
            address  = datamain+str(ticknum) +'win'+str(winnum)+ 'sector'+str(overall-1)+'.pkl'
            print(address)
        ...
        for i in range(weekcount,weekcount+1):
            ...
            f1testavg = np.zeros((30,1))
            for testing in range(30):
                ...
                for r in range(pred_runs):
                    ...
                    adds = mainadd + '/Predictions/Sector' + str(overall) + 'predictions' + str(testing) + 'run' + str(
                        r) + '.pkl'
                ...
                for ti in range(winnum):
                    for f in range(stocks):
                        ...
                        f1testavg[testing, ti] += np.average(f1tempscore) / (stocks * winnum)
