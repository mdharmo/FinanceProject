...
def build_rnn(seqsize,batchsize,labseq):
    ...
    conv3 = ConvLSTM2D(filters=16,kernel_size=(1,2),return_sequences=False)(bn3)
    bn4 = BatchNormalization()(conv3)
    res2 = Flatten()(bn4)

    # Should predict 5 timesteps into the future if I did this correctly...
    outtemp = [Dense(5,activation='softmax',name='confidence_out' + str(i))(res2) for i in range(5)]

    out1 = [Permute([2,1])(RepeatVector(classes)(outtemp[i])) for i in range(labseq)]


    out2 = [TimeDistributed(Dense(5, activation='softmax', name=('predict_out' + str(i))))(res) for i in range(5)]

    out = [markmerge()([out1[i],out2[i]]) for i in range(5)]
...
def main(weekcount,ticknum,winnum):
    ...
    mainadd = '/home/mharmon/FinanceProject/ModelResults/tick91convavg'+str(ticknum) + 'win' + str(winnum)
    address  = '/home/mharmon/FinanceProject/Data/tickdata/train'+str(ticknum) + 'cnn0.pkl'
    batchsize=256
    data,labels,dates = pkl.load(open(address,'rb'))
    seqsize = ticknum
    model,out1 = build_rnn(seqsize,batchsize,labseq)
    month_len = int(8064)
