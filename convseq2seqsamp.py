...
from SampDense import SamplingDense
...
def build_rnn(seqsize,batchsize,labseq):
    ...
    out = [TimeDistributed(SamplingDense(5, activation='softmax', name=('predict_out' + str(i))))(res) for i in range(5)]
...
def main(weekcount,ticknum,winnum,large,restart):
    ...
    mainadd = '/home/mharmon/FinanceProject/ModelResults/tickconvsamp' + str(ticknum) + 'win' + str(winnum)
    ...
    for overall in range(large, 4):
        address = '/home/mharmon/FinanceProject/Data/tickdata/train10win5' + 'sector' + str(
            overall) + '.pkl'
        ...
        if overall > 0 and restart == 1 and weekcount >= 4:
            ...
            print(address)
            old_add = '/home/mharmon/FinanceProject/Data/tickdata/train10win5' + 'sector' + str(
                overall - 1) + '.pkl'
            ...
            old_data = old_data[int(month_len + numtests * week):]
            old_labels = old_data[int(month_len + numtests * week):]
            old_dates = old_data[int(month_len + numtests * week):]
        ...
        for i in range(weekcount,numtests):
            ...
            for r in range(pred_runs):
                ...
                temptesty = np.zeros((week,5,winnum,5))
                for p in range(5):
                ...
                pkl.dump([tempred,temptesty,np.array(test_dates)],open(adds,'wb'))
