...
for j in range(5):
    ...
    for k in range(int(len(data)/end)):
        ...
        featurechunk = np.zeros((500000, 38 * 12)).astype('float32')

        for i in range(len(featurechunk)):
            temp = np.where(data[i+beg-labelrange,-1]==stocks)[0][0]
            first = temp*12
            last = first + 12
    ...
    if j<4:
        ...
        featurechunk = np.zeros((len(datachunk)-labelrange,38*12)).astype('float32')
        for i in range(len(featurechunk)):
            temp = np.where(datachunk[i-labelrange,-1]==stocks)[0][0]
            first = temp*12
            last = first + 12
            featurechunk[i,:] = previousrow
            featurechunk[i,first:last]=data[i,2:14]
        ...
        featurechunk = np.zeros((len(datachunk) - labelrange, 38 * 12)).astype('float32')
        for i in range(len(featurechunk)):
            temp = np.where(data[i-labelrange,-1]==stocks)[0][0]
            first = temp*12
            last = first + 12
            featurechunk[i,:] = previousrow
            featurechunk[i,first:last]=data[i,2:14]
