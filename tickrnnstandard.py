...
def build_rnn(seqsize,batchsize):
    ...
    gru3 = GRU(512,return_sequences=True)(bn2)
    bn3 = BatchNormalization()(gru3)
    gru4 = GRU(5,return_sequences=True)(bn3)
    out = [Activation(activation='softmax',name=('main_output' + str(i)))(gru4) for i in range(5)]
...
def sampling_from_sme(data,lab,seqsize):
    ...
    # should load up the sigma values here for making the other labels
    sigsave = '/home/mharmon/FinanceProject/Data/tickdata/sigma.pkl'
    sigma = pkl.load(open(sigsave,'rb'))
    ...
    for i in range(stocks):
        # only take the last labels of the sequence
        templab = np.argmax(lab[i],axis=2)[:,-1]
            ...
            if j!=neutral:
                ...
                if len(othervec)>0 and (len(othervec)/len(largestvec)<0.5):
                    ...
                    for m in range(stocks):
                        ...
                        if m!=i:
                            vals += factor*(np.argmax(lab[m],axis=2)[othervec,-1]+1)
                            factor*=10
                    ...
                    if len(uniquelabels)>0:
                        for k in range(len(uniquelabels)):
                            ...
                            uselabel = [lab[m][othervec[myvec[0]],-1] for m in range(5)]
                            ...
                            # This gives me the labels at the end of the sequences, but not the entire picture
                            # Need to do a bit of extra work to get all of the new labels
                            if len(tempdata)==0:
                                tempdata = datanew
                                for m in range(5):
                            ...
                            # Make labels within each sequence here
                            for z in range(len(datanew)-1):
                                diff = datanew[z+1]-datanew[z]
...
# Train model here
def main(weekcount):
    address  = '/home/mharmon/FinanceProject/Data/tickdata/train20standard0.pkl'
    mydates = []
    batchsize=256
    ...
    modelsavepath = '/home/mharmon/FinanceProject/ModelResults/tick20/Models/tickmodel.h5'
    ...
    if weekcount>0:
        ...
        f1save = '/home/mharmon/FinanceProject/ModelResults/tick20standard/f1score.npy'
        f1avgsave = '/home/mharmon/FinanceProject/ModelResults/tick20standard/f1avg.npy'
        fwheresave = '/home/mharmon/FinanceProject/ModelResults/tick20standard/fwhere.pkl'
    ...
    for i in range(weekcount, numtests):
        modelsavepath = '/home/mharmon/FinanceProject/ModelResults/tick20standard/Models/tickmodel' + str(i) + '.h5'
        epochx = data[beg:end]
        ...
        adds='/home/mharmon/FinanceProject/ModelResults/tick20standard/Predictions/predictions'+str(i)+'.pkl'
        ...
        figmain = '/home/mharmon/FinanceProject/ModelResults/tick20standard/Figures/HistogramAllWeeksStock'
