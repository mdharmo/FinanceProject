...
def main(weekcount,ticknum,winnum,large,restart):
    ...
    mainadd = '/home/mharmon/FinanceProject/ModelResults/tickconvtest'+str(ticknum) + 'win' + str(winnum)
    batchsize=256
    seqsize = ticknum
    ...
    for overall in range(large,4):
        address  = '/home/mharmon/FinanceProject/Data/tickdata/train'+str(ticknum) + 'cnn'+str(overall)+'.pkl'
        ...
        if overall>0 and restart==0:
            ...
            old_data = data[int(month_len+numtests*week):]
            old_labels = labels[int(month_len+numtests*week):]
            old_dates = dates[int(month_len+numtests*week):]

            data = np.vstack((old_data,newdata))
            labels = np.vstack((old_labels,newlabels))
            dates = np.array([old_dates,newdates])
        ...
        if weekcount<4 and restart==1 and large!=0:
            address  = '/home/mharmon/FinanceProject/Data/tickdata/train'+str(ticknum) + 'cnn'+str(overall-1)+'.pkl'
            ...
            old_data = old_data[int(num_weeks-(4-weekcount)):]
            old_labels = old_labels[int(num_weeks-(4-weekcount)):]
            old_dates = old_dates[int(num_weeks-(4-weekcount)):]
        ...
        modelsavepath = mainadd + '/Models/tickmodel' + str(np.max((0,weekcount-1))) + 'sector'+str(overall)+'.h5'
        ...
        if weekcount==0 and large >0:
            if large<3:
                modelsavepath = mainadd + '/Models/tickmodel' + str(163) + 'sector'+str(overall-1)+'.h5'
            else:
                modelsavepath = mainadd + '/Models/tickmodel' + str(164) + 'sector'+str(overall-1)+'.h5'
