...
def build_ae(seqsize):

    ae = Sequential()
    ae.add(GRU(128,input_shape=(seqsize,5,),return_sequences=True))
    ae.add(BatchNormalization())
    ae.add(GRU(5,return_sequences=True))
    ae.add(BatchNormalization())
    ae.add(GRU(128))
    ae.add(BatchNormalization())
    ae.add(Dense(seqsize*5,activation='linear'))


    ae.compile(optimizer='adadelta', loss='mean_squared_error',metrics=['accuracy'])
...
def build_rnn(seqsize):
    ...
    out = [Dense(5, activation='softmax', name=('main_output' + str(i)))(bn2) for i in range(5)]
    rnn = Model(main_input, out)

    rnn.compile(optimizer='adadelta', loss='categorical_crossentropy',metrics=['accuracy'])
...
# Train model here
def main():
    address  = '/home/mharmon/FinanceProject/Data/tickdata/train20.pkl'
    ...
    for i in range(numtests):
        ...
        for j in range(2):
            ...
            ae.fit(trainx,aetrainx,batch_size=64,epochs=200,validation_data=(valx,aevalx),callbacks=[mc,es])

            # Extract the values I need from the middle of the autoencoder

            ae.model.outputs = [ae.model.layers[4].output]
            newtrainx = ae.model.predict(trainx)
            newvalx = ae.model.predict(valx)
            featsize = len(newtrainx[0,0,:])
        ...
        ae.model.outputs = [ae.model.layers[4].output]
        newtestx = ae.model.predict(testx)
