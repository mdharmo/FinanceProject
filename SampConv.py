...
class SamplingConvLSTM2D(ConvRecurrent2D):
    """Convolutional LSTM.
    ...
    def __init__(self, filters,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(SamplingConvLSTM2D, self).__init__(filters,
                                         kernel_size,
                                         strides=strides,
                                         padding=padding,
        ...
        self.kernel_sigma_initializer=initializers.get(kernel_sigma_initializer)
        self.recurrent_sigma_initializer=initializers.get(recurrent_sigma_initializer)
        ...
        # Here, add everything for the new model type for sigma values
    ...
    def build(self, input_shape):
        ...
        self.kernel_sigma = self.add_weight(shape=kernel_shape,
        ...
        self.recurrent_sigma = self.add_weight(shape=recurrent_kernel_shape,
        ...
        self.kernel_i_sigma=self.kernel_sigma[:, :, :, :self.filters]
        self.recurrent_kernel_i = self.recurrent_kernel[:, :, :, :self.filters]
        self.recurrent_kernel_i_sigma=self.recurrent_sigma[:,:,:,:self.filters]
        self.kernel_f = self.kernel[:, :, :, self.filters: self.filters * 2]
        self.kernel_f_sigma=self.kernel_sigma[:,:,:,self.filters:self.filters*2]
        self.recurrent_kernel_f = self.recurrent_kernel[:, :, :, self.filters: self.filters * 2]
        self.recurrent_kernel_f_sigma=self.recurrent_sigma[:,:,:,self.filters:self.filters*2]
        self.kernel_c = self.kernel[:, :, :, self.filters * 2: self.filters * 3]
        self.kernel_c_sigma=self.kernel_sigma[:,:,:,self.filters*2:self.filters*3]
        self.recurrent_kernel_c = self.recurrent_kernel[:, :, :, self.filters * 2: self.filters * 3]
        self.recurrent_kernel_c_sigma=self.recurrent_sigma[:,:,:,self.filters*2:self.filters*3]
        self.kernel_o = self.kernel[:, :, :, self.filters * 3:]
        self.kernel_o_sigma=self.kernel_sigma[:,:,:,self.filters*3:]
        self.recurrent_kernel_o = self.recurrent_kernel[:, :, :, self.filters * 3:]
        self.recurrent_kernel_o_sigma=self.recurrent_sigma[:,:,:,self.filters*3:]
    ...
    def step(self, inputs, states):
        ...
        x_i = self.input_conv(inputs * dp_mask[0], z_x_i*self.kernel_i_sigma + self.kernel_i, self.bias_i,
                              padding=self.padding)

        z_x_f = tf.random_normal(shape=(K.int_shape(self.kernel_f)))
        x_f = self.input_conv(inputs * dp_mask[1], z_x_f*self.kernel_f_sigma + self.kernel_f, self.bias_f,
                              padding=self.padding)

        z_x_c = tf.random_normal(shape=(K.int_shape(self.kernel_c)))
        x_c = self.input_conv(inputs * dp_mask[2], z_x_c*self.kernel_c_sigma + self.kernel_c, self.bias_c,
                              padding=self.padding)

        z_x_o = tf.random_normal(shape=(K.int_shape(self.kernel_o)))
        x_o = self.input_conv(inputs * dp_mask[3], z_x_o*self.kernel_o_sigma + self.kernel_o, self.bias_o,
                              padding=self.padding)

        z_h_i = tf.random_normal(shape=(K.int_shape(self.recurrent_kernel_i)))
        h_i = self.reccurent_conv(h_tm1 * rec_dp_mask[0],
                                  z_h_i*self.recurrent_kernel_i_sigma + self.recurrent_kernel_i)

        z_h_f = tf.random_normal(shape=(K.int_shape(self.recurrent_kernel_f)))
        h_f = self.reccurent_conv(h_tm1 * rec_dp_mask[1],
                                  z_h_f*self.recurrent_kernel_f_sigma + self.recurrent_kernel_f)

        z_h_c = tf.random_normal(shape=(K.int_shape(self.recurrent_kernel_c)))
        h_c = self.reccurent_conv(h_tm1 * rec_dp_mask[2],
                                  z_h_c*self.recurrent_kernel_c_sigma + self.recurrent_kernel_c)

        z_h_o = tf.random_normal(shape=(K.int_shape(self.recurrent_kernel_o)))
        h_o = self.reccurent_conv(h_tm1 * rec_dp_mask[3],
                                  z_h_o*self.recurrent_kernel_o_sigma + self.recurrent_kernel_o)
    ...
    def get_config(self):
        config = {'activation': activations.serialize(self.activation),
                  ...
                  'recurrent_sigma_constraint': constraints.serialize(self.recurrent_sigma_constraint),
                  'dropout': self.dropout,
                  'recurrent_dropout': self.recurrent_dropout}
        base_config = super(SamplingConvLSTM2D, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))